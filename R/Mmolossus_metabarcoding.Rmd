...
p_load(iNEXT, lme4, car, MASS, lmerTest, effects, multcomp, data.table, 
       corrplot, spaa, tidyverse, dplyr, bipartite, install = TRUE)
```
...
# summarise number of bats of each age and sex class per roost 
180 unique samples
```{r}
data %>% mutate(roost= tolower(roost)) %>% 
  drop_na(sample, roost, sex, Age) -> data 
data %>% group_by(sample, roost, sex, Age, season) %>% 
  summarise(n()) %>% 
  group_by(roost, sex, Age, season) %>% 
  summarise(count = n())-> sum_bat_class
sum(sum_bat_class$count)

sum_bat_class %>% group_by(Age) %>% summarise(total = sum(count))

spread(sum_bat_class %>% group_by(sex, Age) %>% summarise(total = sum(count)), Age, total)
spread(sum_bat_class %>% group_by(sex, season) %>% summarise(total = sum(count)), season, total)
...
# next calcuate curves when prey is only identified to family for both sex and age
```{r}
#SEX
#prepare data
...
f_composition <- data %>% drop_na(sex, Age, Family) %>% 
  filter(diet.status == 'ok') %>%
  filter(Age == 'a') %>%
  dplyr::select(sample, Family, total.diet.reads, sex) %>%
  group_by(sample, Family, sex) %>%
  summarise(reads = sum(total.diet.reads)) %>%
...
###Calculate cz values#
c refers to the between-modules connectivity
z refers to within-module degrees
...
data %>% drop_na(Family, sex, Age) %>% group_by(Family, sex, season) %>% 
  #summarise(count = n()) #-> mfas
  summarise(count = mean(reads/total.diet.reads)) -> mfas
mfas$count %>% hist(breaks = 30)

mfas %>% 
  unite(class, c('sex', 'season'), sep = '.') %>%
  group_by(class, Family) %>%
  summarise(counts = sum(count)) %>%
  pivot_wider(names_from = class, values_from = counts) %>% 
  mutate_if(is.numeric, replace_na, 0) -> network.matrix.m
...
network.mod@originalWeb[order(network.mod@originalWeb[,1], decreasing = TRUE),1] # female dry
network.mod@originalWeb[order(network.mod@originalWeb[,2], decreasing = TRUE),2] # female wet
network.mod@originalWeb[order(network.mod@originalWeb[,3], decreasing = TRUE),3] # male dry
network.mod@originalWeb[order(network.mod@originalWeb[,4], decreasing = TRUE),4] # male wet
```
...
###Plot Modules#
```{r}
plotModuleWeb(network.mod, rank = FALSE)
?plotModuleWeb
```
...
```{r}
data %>% names
data %>% group_by(Family, sex) %>% summarise(count = n()) %>% pivot_wider(names_from=sex, values_from=count) %>% mutate_if(is.numeric, replace_na, 0) %>% mutate(diff = f-m, abs_diff = abs(f-m))-> fam_dif

fam_dif[order(fam_dif$abs_diff, decreasing = TRUE),]
```
